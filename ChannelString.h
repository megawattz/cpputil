...
$Id: ChannelString.h 2428 2012-08-14 15:33:13Z whoward $
*/
...
class ChannelString: public Channel
{
    Stream& _stream;

public:
    ChannelString(const Channel::DIRECTION& direction, Stream& stream) :
    ...
    virtual Text open(const char* package_name = NULL)
    {
        ...
        {
            char buffer[128] = "";
            SerializeArray(*this, buffer, sizeof(buffer), "object");
            return buffer;
        }
    }
    ...
    Text EscapeText(const char* const string, int length = -1) // Convert any non alphanumerics to %XX (http style escape sequences)

    {
        ...
        for (const char* mark(string); *mark != '\0'; ++mark)
            if (not ::isprint(*mark) and (not ispunct(*mark) or strchr("\t\r\n", *mark))) // if *mark is a special character, escape it.
